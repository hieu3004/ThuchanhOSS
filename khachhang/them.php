<?php
   ...
   function postINDEX($index,$values="")
    ...
    $s= " Select email from khachhang WHERE khachhang.email='$email' ";
    $a= $obj->query($s);
    $data= $a->fetchALL();
    print_r($data);
    ...
    if($sm!="")
    {
        ...
        if($pass=="")
        {
            echo $err="mật khẩu không đucợ để trống <br>";
        }
        else if($confimpass=="")
        {
            echo $err="nhập lai mật khẩu không đucợ để trống<br>";
        }
        else{
            if($pass!=$confimpass)
            {
